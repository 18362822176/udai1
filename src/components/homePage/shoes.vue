  ...
  <section class="scroll-floor floor-6">
    ...
    <div class="con-box">
      <a class="left-img hot-img" href>
        <img src="../../assets/images/floor_6.jpg" alt class="cover" />
      </a>
      <div class="right-box">
        <router-link class="floor-item" v-for="item,index in goodsshoeslist" :to="'/goodsShow/'+item.id" :key="index">
          <div class="item-img hot-img">
            <img :src="item.url" class="cover" />
          </div>
          <div class="price clearfix">
            <span class="pull-left cr fz16">￥{{ item.cellPrice }}</span>
            <span class="pull-right c6">进货价</span>
          </div>
          <div class="name ep" :title="item.title">{{ item.title }}</div>
        </router-link>
      </div>
    </div>
  </section>
...
export default {
  ...
  mounted() {
    this.getGoodsShoes({ sort: "鞋靴", num: 8 });
  },
  methods: {
    ...mapActions(["getGoodsShoes"])
  },
  computed: {
    ...mapState(["goodsshoeslist"])
  }
};
</script>
<style scoped>
